...
$kinphonelength = strlen($kinphonenumber);
if (
    empty($firstname) || empty($othernames) || empty($phonenumber) || empty($emailaddress) || empty($idnumber) ||
    empty($station_name) || empty($residence) || empty($kinfullname) || empty($kinphonenumber) || empty($username) || empty($password)
) {
    $message = "
<script>
...
";
} else if ($phonelength !== 10 || $kinphonelength !== 10) {
    $message = "
<script>
